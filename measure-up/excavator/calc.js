/**
 * Siteworks Excavator measure-up — machine frame from G, CT1, CT2.
 */
var ExcavatorMeasureUpCalc = (function () {
  var RESULT_KEYS = [
  ...
  function calculateForWeb(
    ...
    var csvData = parseCsvString(csvString);
    if (!csvData || csvData.length < 1) throw new Error('CSV file is empty.');
    ...
    if (hasHeader) {
      ...
      if (idxN === -1 || idxE === -1 || idxZ === -1 || idxName === -1) {
        throw new Error('Header detected but required columns are missing.');
      }
    } else {
      idxName = 0;
      idxZ = 3;
    ...
    if (missing.length) throw new Error('Missing points: ' + missing.join(', '));
    ...
    if (mag === 0) throw new Error('CT1 and CT2 cannot be in the same horizontal position.');
    ...
    ['G', 'CT1', 'CT2', 'CL', 'BB', 'BL', 'BR', 'CR'].forEach(function (key) {
      var pt = getPt(key);
    ...
    if (centerlineMethod === 'Manual') {
      recBracketToCenterline = (
        parseFloat(manualCenterlineVal) / 2 +
        parseFloat(manualCenterlineOffset)
      ).toFixed(3);
    } else if (BL && BR) {
      recBracketToCenterline = (Math.abs(BR.e - BL.e) / 2 + manualOffset).toFixed(3);
    } else {
    ...
    // Attachment triangle in machine frame: hypotenuse G→CL, horizontal = |CL.n|
    ...
    } else if (widthMethod === 'Total Station') {
      var rawCL = getPt('CL');
      var rawCR = getPt('CR');
      attachmentWidth = Math.sqrt(
        Math.pow(rawCR.e - rawCL.e, 2) +
          Math.pow(rawCR.n - rawCL.n, 2) +
          Math.pow(rawCR.z - rawCL.z, 2)
      ).toFixed(3);
    } else {
      attachmentWidth = 'Error';
    }

    var res = {
      'Receiver bracket bolt to pivot point': bracketBoltToPivot,
      'Reciever Bracket to Centerline': recBracketToCenterline,
      'Pivot point to plumb bob': pivotToPlumb,
      'Pivot Point to Attachment Cutting Edge': pivotToCuttingEdge,
      'Attachment Cutting Edge to Plumb Bob': cuttingEdgeToPlumb,
      'Attachment Width': attachmentWidth,
    };

    return {
      calculations: res,
      meta: { time: new Date().toLocaleString(), units: units, machine: 'Excavator' },
  ...
  function parseSurveyPoints(text, csvFormat) {
    ...
    if (!rows.length) return { foundPoints: {}, error: 'Empty file' };
    ...
    if (hasHeader) {
      ...
      if (idxN === -1 || idxE === -1 || idxZ === -1 || idxName === -1) {
        return { foundPoints: {}, error: 'Header columns not identified' };
      }
    } else {
      idxName = 0;
      idxZ = 3;
    ...
    return { foundPoints: foundPoints, error: null };
  }
