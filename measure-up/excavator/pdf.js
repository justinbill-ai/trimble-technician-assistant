/**
 * Excavator measure-up PDF — matches CTL / pre-inspection report layout.
 */
var ExcavatorMeasureUpPdf = (function () {
  var MACHINE_KEYS = [
  ...
  function buildPdfHeaderHtml(data) {
    ...
    var subtitle =
      data.reportName && data.reportName.trim()
        ? escapeHtml(data.reportName.trim())
        : 'Siteworks Excavator measure-up';
    ...
    return (
      ...
      dealerSlot +
      '</div></div>' +
      '<h1>Excavator Measure-Up Report</h1>' +
      '<p>' +
      subtitle +
      ' · Generated ' +
  ...
  function buildHtml(data) {
    ...
    return (
      ...
      escapeHtml(data.reportTitle || 'Excavator Measure-Up Report') +
      "</title>" +
      ...
      row('Machine type', meta.machine || 'Excavator') +
      ...
      '<div class="footer">Trimble Technician Assistant — Excavator Measure-Up Report (BETA). Field survey calculator for Siteworks machine guidance.</div>' +
      '</div></body></html>'
    );
  }
