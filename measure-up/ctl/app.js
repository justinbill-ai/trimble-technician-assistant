...
function buildSectionRowHtml(title) {
  return '<tr class="mu-res-section"><td colspan="3">' + escapeHtmlResult(title) + '</td></tr>';
}
...
function parseCSVForPreview(file) {
  ...
  reader.onload = function (e) {
    ...
    var currentRequired = ['BB', 'CT1', 'CT2', 'G', 'CL'];
  ...
  reader.onload = function (e) {
    try {
      ...
      MACHINE_RESULT_KEYS.forEach(function (rk) {
        if (data[rk] != null) body.innerHTML += buildResultRowHtml(rk, data[rk]);
      });
...
function bindMeasureUpUi() {
  ...
  syncMeasureUpGate(false);
  if (typeof window.CtlMeasureNotesInit === 'function') window.CtlMeasureNotesInit();
}
